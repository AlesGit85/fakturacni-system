...
class InvoicesPresenter extends BasePresenter
{
    ...
    public function actionPdf(int $id): void
    {
        ...
        // Převod HEX na RGB
        $headingColor = $this->hex2rgb($headingColorHex);
        $trapezoidBgColor = $this->hex2rgb($trapezoidBgColorHex);
        $trapezoidTextColor = $this->hex2rgb($trapezoidTextColorHex);
        $labelsColor = $this->hex2rgb($labelsColorHex);
        $footerColor = $this->hex2rgb($footerColorHex);
        ...
        foreach ($invoiceItems as $key => $item) {
            ...
            $nameLines = $this->wrapText($item->name, 100);
            ...
            if (!empty($item->description)) {
                ...
                $descLines = $this->wrapText($item->description, 100);
     ...
     * (KOPIE kódu z actionPdf, jen s jiným výstupem a bez načítání dat)
     */
    private function generateInvoicePdfForEmail($invoice, $client, $company, string $outputPath): void
    {
        // ✅ Data už máme načtená z parametrů, nemusíme je znovu načítat!
        ...
        // ✅ ROZDÍL: Výstup do souboru místo download
        $pdf->Output($outputPath, 'F');
    }

    /**
     * Převod HEX barvy na RGB hodnoty
     */
    ...
    /**
     * Inteligentní zalamování textu na max. počet znaků
     */
    private function wrapText(string $text, int $maxLength = 100): array
    {
        if (empty($text)) {
            return [];
        }
